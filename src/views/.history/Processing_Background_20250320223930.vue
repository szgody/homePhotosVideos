        ...
        for (const file of data.files) {
          ...
          while (retryCount < maxRetries && !success) {
            try {
              ...
              if (retryCount > 0) {
                addLog('warning', `重试处理视频 (${retryCount}/${maxRetries}): ${file}`);
              } else {
              ...
              if (retryCount >= maxRetries) {
                addLog('error', `处理视频失败(已重试${maxRetries}次): ${file} - ${error.message}`);
              } else {
                addLog('warning', `处理视频出错，准备重试(${retryCount}/${maxRetries}): ${error.message}`);
                // 短暂等待后重试
