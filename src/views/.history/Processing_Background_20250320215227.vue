<template>
  ...
  <div class="processing-background">
    ...
    <div class="control-panel">
      ...
      <button @click="stopProcessing" v-if="processing" class="stop-btn">
        ...
        try {
          ...
          if (!snResponse.ok) {
            if (snResponse.status === 404) {
              // 如果文件不存在，使用默认序号
              addLog("warning", "视频序号文件不存在，使用默认序号: 000001");
            ...
            currentNumber = await snResponse.text();
            ...
            if (!/^\d+$/.test(currentNumber)) {
              addLog(
                "warning",
                `序号格式无效: ${currentNumber}，使用默认序号: 000001`,
              );
              currentNumber = "000001";
            } else {
              // 确保6位数字格式
          ...
          addLog(
            "error",
            `读取视频序号失败: ${error.message}，使用默认序号: 000001`,
          );
          currentNumber = "000001";
        }
    ...
    const checkOngoingProcessing = async () => {
      try {
        ...
        if (data && data.isProcessing) {
          ...
          addLog(
            "info",
            `恢复${processingType.value === "image" ? "图片" : "视频"}处理进度: ${progress.value}%`,
          );
          return true;
        }
        return false;
...
// 在express应用的routes文件中添加 app.post('/api/write-video-sn', (req, res) =>
{ const { sn } = req.body; if (!sn || !/^\d{1,6}$/.test(sn)) { return
res.status(400).json({ success: false, error: '无效的序号格式' }); } const
videoSnPath = path.join(__dirname, '../data/videos/sn.txt'); try { //
确保序号为6位数字 const formattedSn = sn.padStart(6, '0');
fs.writeFileSync(videoSnPath, formattedSn, 'utf8'); res.json({ success: true });
} catch (error) { console.error('写入视频序号失败:', error);
