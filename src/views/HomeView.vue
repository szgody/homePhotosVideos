  ...
  <div class="home">
    ...
    <div class="featured-photos">
      ...
      <div class="featured-grid">
        <div v-for="photo in displayPhotos.slice(0, 2)" 
          ...
          <img :src="photo.photo || photo.thumbnail"
               :alt="photo.filename"
    ...
    <div v-if="previewVisible" class="preview-overlay" @click="closePreview">
      <div class="preview-container">
        <img :src="previewPhoto?.photo || `${apiUrl.value}${photosPath.value}/${previewPhoto?.filename}`" 
...
export default {
  ...
  setup() {
    ...
    const videoThumbnailsPath = ref(import.meta.env.VITE_VIDEO_THUMBNAILS_PATH || '/video_thumbnails')
    const photoThumbnailsPath = ref(import.meta.env.VITE_PHOTO_THUMBNAILS_PATH || '/photo_thumbnails')
    ...
    const fetchLatestVideos = async () => {
      try {
        ...
        latestVideos.value.forEach(video => {
          if (video.thumbnailPath && video.thumbnailPath.startsWith(apiUrl.value)) {
            // 替换为相对路径 Replace with relative path
            video.thumbnailPath = video.thumbnailPath.replace(new RegExp(`^${apiUrl.value}`), '')
          }
        })
      } catch (error) {
        console.error('获取最新视频失败 Failed to fetch latest videos:', error.message)
        latestVideos.value = []
      }
    }
    ...
    const handlePreviewError = (e) => {
      console.error('预览图片加载失败 Preview image load failed:', e.target.src)
      // 尝试使用缩略图
