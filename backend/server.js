...
const PATHS = {
  ...
  // 确保这些配置与 .env 文件一致
  ORIGINAL_IMAGES: path.join(__dirname, '..', process.env.PUBLIC_DIR || 'public', 'original', 'images'),
  ORIGINAL_VIDEOS: path.join(__dirname, '..', process.env.PUBLIC_DIR || 'public', 'original', 'videos'),
  
  PHOTOS: path.join(__dirname, '..', process.env.DATA_DIR || 'data', 'photos'),
  PHOTO_THUMBNAILS: path.join(__dirname, '..', process.env.DATA_DIR || 'data', 'photo_thumbnails'), 
  
  VIDEOS: path.join(__dirname, '..', process.env.DATA_DIR || 'data', 'videos'),
  VIDEO_THUMBNAILS: path.join(__dirname, '..', process.env.DATA_DIR || 'data', 'video_thumbnails'),
  
  // 序列号文件存储路径
  PHOTO_SN_FILE: path.join(__dirname, '..', process.env.DATA_DIR || 'data', 'photos', 'sn.txt'),
  VIDEO_SN_FILE: path.join(__dirname, '..', process.env.DATA_DIR || 'data', 'videos', 'sn.txt')
};
...
app.use(process.env.PHOTO_THUMBNAILS_PATH || '/photo_thumbnails', express.static(PATHS.PHOTO_THUMBNAILS)); // 缩略图 Thumbnails
...
app.use(process.env.VIDEO_THUMBNAILS_PATH || '/video-thumbnails', express.static(PATHS.VIDEO_THUMBNAILS)); // 视频缩略图 Video thumbnails
...
app.post('/api/process-single-video', async (req, res) => {
  try {
    ...
    // 删除原始文件
    await fs.remove(path.join(PATHS.ORIGINAL_VIDEOS, filename));
...
app.post('/api/write-sn', async (req, res) => {
  try {
    ...
    if (verificationSn.trim() !== formattedSN) {
      throw new Error(`写入验证失败: 期望 "${formattedSN}" 但得到 "${verificationSn.trim()}"`);
    }
...
const processImage = async (filename, newName) => {
  try {
    ...
    const targetPath = path.join(PATHS.PHOTOS, `${newName}${path.extname(filename)}`);
    const thumbnailPath = path.join(PATHS.PHOTO_THUMBNAILS, `${newName}${path.extname(filename)}`);
      ...
      const ffmpegCommand = ffmpeg(originalPath)
        ...
        .on('progress', (progress) => {
          if (progress.percent && Math.floor(progress.percent) > lastProgress) {
            ...
            if (!global.videoProgressData[filename]) global.videoProgressData[filename] = {};

            global.videoProgressData[filename].percent = lastProgress;
            global.videoProgressData[filename].timemarks = progress.timemark || '';
...
app.post('/api/process-single-image', async (req, res) => {
  ...
  try {
    console.log(`开始处理图片: ${filename} -> ${newName}, 内存使用: ${JSON.stringify(process.memoryUsage())}`);
    ...
    console.log(`图片处理完成: ${filename}, 用时: ${processTime}ms, 内存使用: ${JSON.stringify(process.memoryUsage())}`);
...
app.get('/api/read-sn', async (req, res) => {
  try {
    ...
    if (!exists) {
      console.log(`序号文件不存在，创建默认文件 File doesn't exist, creating default`);
      // 创建包含默认序号的文件
    ...
    // 验证序号格式
    if (!/^\d+$/.test(sn)) {
      console.log(`序号格式无效: "${sn}", 使用默认值 Invalid SN format, using default`);
      sn = '000000';
      // 写入默认值
...
app.post('/api/write-sn', async (req, res) => {
  try {
    ...
    if (verificationSn.trim() !== sn) {
      throw new Error(`写入验证失败: 期望 "${sn}" 但得到 "${verificationSn.trim()}"`);
    }
...
app.get('/api/video-progress', (req, res) => {
  ...
  const progressData = global.videoProgressData && global.videoProgressData[filename] 
    ? global.videoProgressData[filename] 
  ...
  try {
    ...
    for (const file of videoFiles) {
      try {
        ...
        console.error(`删除视频文件 ${file} 失败:`, error);
        errors.push({ file, error: error.message });
      }
    }
    ...
    res.json({
      ...
      totalFiles: videoFiles.length,
      errors: errors.length > 0 ? errors : undefined,
      message: `已成功删除 ${deletedCount} 个原始视频`
    });
...
server.listen(port, () => {
  ...
  console.log('- 缩略图路径:', process.env.THUMBNAILS_PATH || '/photo_thumbnails');
  ensureGlobals();
});
...
app.get('/api/processing-status', (req, res) => {
  res.json(global.processingStatus);
});
...
const processImages = async (req, res) => {
  try {
    ...
    // 在处理过程中更新状态
    global.processingStatus.currentFile = file;
    global.processingStatus.originalName = file;
    global.processingStatus.newName = newName;
    global.processingStatus.currentSN = sn;
    global.processingStatus.currentStage = stage;
    global.processingStatus.processedCount = processedCount;
    global.processingStatus.totalCount = totalCount;
    global.processingStatus.progress = progress;
...
// 同样为处理视频的函数添加状态更新
const processVideos = async (req, res) => {
  // 类似的实现...
};
...
const diagnoseApiConnection = async () => {
  ...
  const apiBasePath = apiUrl.value.replace(/\/api\/?$/, '');
  
  for (const path of possiblePaths) {
    try {
      const fullPath = `${apiBasePath}${path}`;
      addLog('info', `尝试访问 API 路径: ${fullPath}`);
      ...
      if (status === 200) {
        addLog('success', `找到可用的 API 路径: ${fullPath} (状态码: ${status})`);
        
        try {
          const data = await response.text();
          addLog('info', `API 返回数据: ${data.substring(0, 100)}${data.length > 100 ? '...' : ''}`);
        } catch (e) {
          addLog('info', `无法解析返回数据: ${e.message}`);
        }
      } else {
        addLog('warning', `API 路径 ${fullPath} 返回状态码: ${status}`);
      }
    } catch (error) {
      addLog('error', `访问 ${apiBasePath}${path} 失败: ${error.message}`);
    }
  }
  ...
  for (const endpoint of endpoints) {
    try {
      const fullPath = `${apiUrl.value}${endpoint}`;
      addLog('info', `尝试访问端点: ${fullPath}`);
      
      const response = await fetch(fullPath, {
        headers: { 'Cache-Control': 'no-cache' }
      });
      
      addLog(response.ok ? 'success' : 'warning', 
             `端点 ${endpoint} 返回状态码: ${response.status}`);
    } catch (error) {
      addLog('error', `访问 ${apiUrl.value}${endpoint} 失败: ${error.message}`);
    }
  }
};

// 在express应用的routes文件中添加
app.get('/api/read-video-sn', (req, res) => {
  const videoSnPath = path.join(__dirname, '../data/videos/sn.txt');
  
  // 检查文件是否存在
  if (!fs.existsSync(videoSnPath)) {
    // 如果文件不存在，创建一个新的序号文件，从"000001"开始
    fs.writeFileSync(videoSnPath, '000001', 'utf8');
    return res.send('000001');
  }
  
  try {
    // 读取序号文件
    const sn = fs.readFileSync(videoSnPath, 'utf8').trim();
    res.send(sn);
  } catch (error) {
    console.error('读取视频序号失败:', error);
...
app.post('/api/write-video-sn', (req, res) => {
  ...
  if (!sn || !/^\d{1,6}$/.test(sn)) {
    return res.status(400).json({ success: false, error: '无效的序号格式' });
  }
  ...
  try {
    ...
    fs.writeFileSync(videoSnPath, formattedSn, 'utf8');
    res.json({ success: true });
  } catch (error) {
    console.error('写入视频序号失败:', error);
    res.status(500).json({ success: false, error: '写入视频序号失败' });
  }
});