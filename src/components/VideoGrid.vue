<template>
  ...
  <div class="video-container">
    ...
    <div class="video-grid">
      ...
      <div v-for="video in displayVideos" 
           :key="video.filename" 
           class="video-item"
  ...
  <div v-if="showVideoPlayer" class="video-modal" @click="closeVideoPlayer">
    <div class="video-player-container" @click.stop>
      <!-- 视频标题 Video title -->
      <h3 v-if="currentVideo">{{ currentVideo.name || currentVideo.filename }}</h3>
  ...
  computed: {
    ...
    displayVideos() {
      ...
      if (this.sortBy === 'date') {
        ...
        videosCopy.sort((a, b) => {
          // 使用视频的创建日期或修改日期进行排序
          const dateA = new Date(a.created_at || a.date || a.timestamp || a.mtime || 0);
          const dateB = new Date(b.created_at || b.date || b.timestamp || b.mtime || 0);
