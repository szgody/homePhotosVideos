  ...
  setup() {
    ...
    const processImages = async () => {
      ...
      try {
        ...
        // 读取当前序号
        try {
          const snResponse = await fetch('http://localhost:3000/api/read-sn')
          if (!snResponse.ok) {
        ...
        for (const file of data.files) {
          try {
            ...
            const processResponse = await fetch('http://localhost:3000/api/process-single-image', {
              method: 'POST',
        ...
        // 写入新序号
        const writeResponse = await fetch('http://localhost:3000/api/write-sn', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    const startProgressMonitoring = () => {
      eventSource = new EventSource('http://localhost:3000/api/progress-stream');
    ...
    const processVideos = async () => {
      try {
        ...
        // 获取视频列表
        const response = await fetch('http://localhost:3000/api/list-videos');
        if (!response.ok) {
        ...
        // 读取当前序号
        const snResponse = await fetch('http://localhost:3000/api/read-video-sn', {
          method: 'POST'
        })
        ...
        for (const file of data.files) {
          try {
            ...
            const processResponse = await fetch('http://localhost:3000/api/process-single-video', {
              method: 'POST',
              headers: {
        ...
        if (processedCount.value > 0) {
          const writeResponse = await fetch('http://localhost:3000/api/write-video-sn', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
